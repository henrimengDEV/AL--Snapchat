...
class BodyChat extends StatelessWidget {
          ...
          children: [
            ...
            StreamBuilder<List<ConversationFirebase>>(
              ...
              builder: (context, document) {
                if (document.hasError) return Text("Error : ${document.error}");
                if (!document.hasData) {
  ...
  Future<QuerySnapshot<Map<String, dynamic>>> _user(
          ConversationFirebase conversation, BuildContext context) =>
      FirebaseFirestore.instance
          .collection("users")
          .where(
            "id",
            isEqualTo: conversation.users.firstWhere(
              (element) =>
                  element != context.read<SessionBloc>().state.user!.id,
            ),
          )
          .snapshots()
          .first;
